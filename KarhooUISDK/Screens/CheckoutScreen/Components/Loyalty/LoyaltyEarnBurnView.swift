...
struct LoyaltyEarnBurnView: View {
    ...
    @StateObject var viewModel = NewLoyaltyViewModel(worker: TempLoyaltyWorker())
...
class NewLoyaltyViewModel: ObservableObject {
    ...
    let onValue: (Bool?) -> Void = { value in
        
    }
}

class TempLoyaltyWorker {
    var loyaltyStatusSubject = CurrentValueSubject<Bool?, LoyaltyErrorType>(nil)
